...
/** Traversal steps for MacroRef */
class MacroRefTraversalExtGen[NodeType <: MacroRef](val traversal: Iterator[NodeType]) extends AnyVal {
  ...
  /** Traverse to macroFullName property */
  def macroFullName: Iterator[String] =
    traversal.map(_.macroFullName)

  /** Traverse to nodes where the macroFullName matches the regular expression `value`
    */
  def macroFullName(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      macroFullNameExact(pattern)
    } else {
      overflowdb.traversal.filter.StringPropertyFilter.regexp(traversal)(_.macroFullName, pattern)
    }
  }

  /** Traverse to nodes where the macroFullName matches at least one of the regular expressions in `values`
    */
  def macroFullName(patterns: String*): Iterator[NodeType] =
    overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(traversal)(_.macroFullName, patterns)

  /** Traverse to nodes where macroFullName matches `value` exactly.
    */
  def macroFullNameExact(value: String): Iterator[NodeType] = {
    val fastResult = traversal match {
      case init: overflowdb.traversal.InitialTraversal[NodeType] =>
        init.getByIndex("MACRO_FULL_NAME", value).getOrElse(null)
      case _ => null
    }
    if (fastResult != null) fastResult
    else traversal.filter { node => node.macroFullName == value }
  }

  /** Traverse to nodes where macroFullName matches one of the elements in `values` exactly.
    */
  def macroFullNameExact(values: String*): Iterator[NodeType] = {
    if (values.size == 1)
      macroFullNameExact(values.head)
    else
      overflowdb.traversal.filter.StringPropertyFilter
        .exactMultiple[NodeType, String](traversal, node => Some(node.macroFullName), values, "MACRO_FULL_NAME")
  }

  /** Traverse to nodes where macroFullName does not match the regular expression `value`.
    */
  def macroFullNameNot(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      traversal.filter { node => node.macroFullName != pattern }
    } else {
      overflowdb.traversal.filter.StringPropertyFilter.regexpNot(traversal)(_.macroFullName, pattern)
    }
  }

  /** Traverse to nodes where macroFullName does not match any of the regular expressions in `values`.
    */
  def macroFullNameNot(patterns: String*): Iterator[NodeType] = {
    overflowdb.traversal.filter.StringPropertyFilter.regexpNotMultiple(traversal)(_.macroFullName, patterns)
  }
