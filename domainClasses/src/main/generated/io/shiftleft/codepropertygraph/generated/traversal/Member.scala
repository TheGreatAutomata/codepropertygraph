...
class MemberTraversalExtGen[NodeType <: Member](val traversal: Iterator[NodeType]) extends AnyVal {
  ...
  /** Traverse to memberOffset property */
  def memberOffset: Iterator[scala.Int] =
    traversal.map(_.memberOffset)

  /** Traverse to nodes where the memberOffset equals the given `value`
    */
  def memberOffset(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset == value }

  /** Traverse to nodes where the memberOffset equals at least one of the given `values`
    */
  def memberOffset(values: scala.Int*): Iterator[NodeType] = {
    val vset = values.toSet
    traversal.filter { node => vset.contains(node.memberOffset) }
  }

  /** Traverse to nodes where the memberOffset is greater than the given `value`
    */
  def memberOffsetGt(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset > value }

  /** Traverse to nodes where the memberOffset is greater than or equal the given `value`
    */
  def memberOffsetGte(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset >= value }

  /** Traverse to nodes where the memberOffset is less than the given `value`
    */
  def memberOffsetLt(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset < value }

  /** Traverse to nodes where the memberOffset is less than or equal the given `value`
    */
  def memberOffsetLte(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset <= value }

  /** Traverse to nodes where memberOffset is not equal to the given `value`.
    */
  def memberOffsetNot(value: scala.Int): Iterator[NodeType] =
    traversal.filter { _.memberOffset != value }

  /** Traverse to nodes where memberOffset is not equal to any of the given `values`.
    */
  def memberOffsetNot(values: scala.Int*): Iterator[NodeType] = {
    val vset = values.toSet
    traversal.filter { node => !vset.contains(node.memberOffset) }
  }
